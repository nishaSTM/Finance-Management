...
public class ExpenseDatabase {
	...
	static String[][] tables3 = new String[][] { { "_id", "note",
		"transaction_date","typeOfItem","sourceOfPayment","money" } };
	...
	private static final String TABLE_3_CREATE = "create table "
			...
			+ " text , " + tables3[EXPENSE_INT][2]
					+ " TIMESTAMP DEFAULT CURRENT_TIMESTAMP ," + tables3[EXPENSE_INT][3]+ " text," + tables3[EXPENSE_INT][4]+ " text,"+ tables3[EXPENSE_INT][5]+ " text"+ ");";
...
/*	public long createAlert(String DATABASE_TABLE, int tableNo, int id,
			String[] values) {
        ContentValues vals = new ContentValues();
		vals.put(tables[tableNo][0], id);
		for (int i = 0; i < values.length; i++) {
			vals.put(tables[tableNo][i + 1], values[i]);
		}
		return sqLiteDb.insert(DATABASE_TABLE, null, vals);
